...
def _editor_prompt(context: str, run_date: str) -> str:
    return (
        ...
        "- every story has a specific heading that states its result or change;\n"
        "- every story makes what happened and why it matters unmistakable and normally uses "
        "visible **What happened:** and **Why it matters:** labels;\n"
        "- every story uses **Bigger picture:** only when a real wider effect exists; keep it to "
        "one honest sentence or omit it when the effect is narrow or unknown;\n"
        "- every story includes a concrete example when it makes the idea easier to understand, "
        "but does not pad a label, invent a market effect, or force an example;\n"
        "- every section works on its own; for an unfamiliar mechanism, start at the lowest "
        "missing foundation and include only the steps needed to understand it: a concrete action "
        "or example, a problem before the change that handles it, and the evidence or limit;\n"
        "- short or non-mechanism stories stay short when a few sentences already explain them; "
        "do not force every causal step or alternative into every story;\n"
        "- when a design has several parts, introduce each part after the problem it solves; "
        "mention an alternative only when it changes the reader's understanding or choice, and "
        "put technical names after the things they name make sense;\n"
        "- no sentence compresses several steps into abstract nouns, and every source-specific "
        "benchmark, method, or uncommon term has the shortest reminder needed to follow it;\n"
